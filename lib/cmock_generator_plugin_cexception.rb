...
class CMockGeneratorPluginCexception
  ...
  def mock_function_declarations(function)
    args_string, call_args_string = @utils.args_to_s(function[:args], true, "exception")
    if (args_string == "void")
      ...
      return "#define #{function[:name]}_ExpectAndThrow(#{function[:args_call]}, cmock_to_throw) #{function[:name]}_CMockExpectAndThrow(__LINE__, #{function[:args_call]}, cmock_to_throw)\n" +
             "void #{function[:name]}_CMockExpectAndThrow(UNITY_LINE_TYPE cmock_line, #{args_string}, CEXCEPTION_T cmock_to_throw);\n"
    end
  end
  ...
  def mock_interfaces(function)
    arg_insert = (function[:mock_args_string] == "void") ? "" : "#{function[:mock_args_string]}, "
    call_args_string = function[:args].map{|m| m[:name]}.join(', ')
    [ "void #{function[:name]}_CMockExpectAndThrow(UNITY_LINE_TYPE cmock_line, #{arg_insert}CEXCEPTION_T cmock_to_throw)\n{\n",
      @utils.code_add_base_expectation(function),
      @utils.code_call_argument_loader(function,"exception"),
