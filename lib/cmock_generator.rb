...
class CMockGenerator
  ...
  def create_instance_structure(file, functions)
    ...
    functions.each do |function|
      ...
      function[:args].reject{|x| !(x[:extra?] and x[:ptr?])}.each do |arg|
        file << "  CMOCK_MEM_INDEX_TYPE #{function[:name]}_ArrayInstance_#{arg[:name]};\n"
      end  
    end
    file << "} Mock;\n\n"
  end
  ...
  # the mock implementation - #{function_name}(#{args}) 
  def create_mock_implementation(file, function)
    ...
    ret = ""
    ret = "(#{function_mod_and_rettype})0" if function_mod_and_rettype.to_s.downcase != "void"
    args_string, call_args_string = @utils.args_to_s(function[:args], false)
    ...
    file << "  if (cmock_call_instance == NULL) return #{ret};\n"
  ...
  # create 3 functions:
  #     Public: XXX_CMockExpectWithArrayAndReturn and XXX_CMockExpectAndReturn
  #     private: CMockExpectParameters_XXX
