...
class CMockHeaderParser
  ...
  def import_source(source)
    ...
    #replace all "VOID" to "void"
    source.gsub!(/PVOID/,"void *")
    source.gsub!(/VOID/,"void")
  ...
  def parse_args_ex(arg_list)
    ...
    arg_list.split(',').each do |arg|
      ...
      arg_type =arg_elements[0..-2].join(' ')
      arg_name = arg_elements[-1]
      arg_const = arg_array.include?('const')


      # for ptr arg, need to add an extra arg to store the value expect to return.
      # For example, int** i => int ** i, int ret_i
      #              int * => int *i, int i_Depth, int* i_Val
      args << { :type   => arg_type,
                :name   => arg_name,
                :ptr?   => divine_ptr(arg_type),
                :ptrptr? => divine_ptrptr(arg_type),
                :const? => arg_const,
                :extra? => false,
                :depth_name => "",
              }

      if divine_ptrptr(arg_type)
        args << {:type => arg_type.sub("*",""),
                 :name => "#{arg_name}_Val",
                 :ptr?  => false,
                 :ptrptr => false,
                 :const? => arg_const,
      ...
      elsif divine_ptr(arg_type)

        args << {:type => "int",#arg_elements[0..-2].collect{|x| x.gsub("*","")}.join(' '),
                 :name => "#{arg_name}_Depth",
                 :ptr?  => false,
                 :ptrptr => false,
                 :const? => arg_const,
                 :extra? => true,
                 :depth_name => nil # int * but only check point, not content
              } 
        args << {:type => arg_type,
                 :name => "#{arg_name}_Val",
                 :ptr?  => true,
                 ...
                 :const? => arg_const,
                 :extra? => true,
                 :depth_name => arg_name + "_Depth",  # int *, check content.
              }
      end        
    end
    return args
  ...
  def parse_declaration(declaration)
    ...
    decl[:args] = parse_args_ex(args)
    #TODO: args's value can be changed, for example, int **p
    decl[:args_string] = decl[:args].map{|a| "#{a[:type]} #{a[:name]}"}.join(', ')
